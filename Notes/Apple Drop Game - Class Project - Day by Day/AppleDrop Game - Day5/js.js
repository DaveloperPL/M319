...
function initialize() {
    ...
    for (let i = 0; i < 5; i++) {
        appleArray.push(createImage(apple.src,Math.floor(Math.random()*400),Math.floor(Math.random()*400),25,25,true))
    }
    lightning =createImage(lightning.src,Math.floor(Math.random()*400),50,25,25,true)
...
function drawApple(){
    ...
    for (let i = 0; i < appleArray.length; i++) {
        ctx.drawImage(appleArray[i],appleArray[i].xLoc,appleArray[i].yLoc,25,25);
        appleArray[i].yLoc+=5
        if(appleArray[i].yLoc >500){
            appleArray[i].xLoc = Math.floor(Math.random()*400)
            appleArray[i].yLoc = Math.floor(Math.random()*-50)
        }
    }
...
function checkCollisionApples(){
    for (let i = 0; i < appleArray.length; i++) {
        if (basketX<appleArray[i].xLoc+appleArray[i].width && basketX+75>appleArray[i].xLoc &&
            basketY<appleArray[i].yLoc+appleArray[i].height && basketY+75>appleArray[i].yLoc){
            score = score+1;
            appleArray[i].xLoc = Math.floor(Math.random()*400)
            appleArray[i].yLoc = Math.floor(Math.random()*-50)
        }
